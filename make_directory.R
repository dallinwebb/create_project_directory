...
group_project_dir <- function(members, readme_help = TRUE, workflow_help = TRUE) {
  require(tidyverse)
  require(fs)
  
  members    <- members %>% str_replace_all("\\s", "_")
  dir_main   <- c("data","deliverables", "documents", "global_scripts", "personal_directories")
  dir_readme <- c(paste0("personal_directories", "/", members, "/", "README.md"),
                paste0(c(dir_main, "personal_directories"), "/", "README.md"))
  
  members %>% dir_create(path = "personal_directories")
  dir_main %>% dir_create()
...
group_project_dir(members, readme_help = FALSE)

dir_delete(path = c("data", "documents","deliverables", "global_scripts", 
